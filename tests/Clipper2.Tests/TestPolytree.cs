...
{
  /// <summary>
  /// Ports of the polytree tests (CPP/Tests/TestPolytreeHoles.cpp &amp; friends).
  /// </summary>
  [TestClass]
  public class TestPolytree
  {
    ...
    {
      foreach (PolyPathBase childBase in pp)
      {
        PolyPath64 child = (PolyPath64) childBase;
    ...
    public void TestPolytree2()
    {
      ...
      Assert.IsTrue(ClipperFileIO.LoadTestNum(TestData.Path("PolytreeHoleOwner2.txt"),
      ...
      foreach (Point64 pt in pointsOfInterestInside)
      {
        ...
        Assert.IsTrue(poiInsideCounter == 1,
          string.Format("poi_inside_counter - expected 1 but got {0}", poiInsideCounter));
      }

      clipper.AddSubject(subject);
      clipper.AddOpenSubject(subjectOpen);
      clipper.AddClip(clip);
      clipper.Execute(cliptype, fillrule, solutionTree, solutionOpen);
