...
*                                                                              *
* C# port of CPP/Utils/clipper.svg.h + src/clipper.svg.cpp                     *
*******************************************************************************/
...
#endif
{
  ...
  public class SvgReader
  {
    ...
    private static bool SkipBlanks(string xml, ref int i, int endI)
    {
      while (i < endI && xml[i] <= ' ') i++;
      return i < endI;
    }

    private static bool SkipOptionalComma(string xml, ref int i, int endI)
    {
      while (i < endI && xml[i] <= ' ') i++;
      if (i < endI && xml[i] == ',') i++;
      else return (i < endI);
    ...
    private static bool GetNum(string xml, ref int i, int endI, out double val)
    {
      while (i < endI && xml[i] <= ' ') i++;
      bool isneg = i < endI && xml[i] == '-';
      if (isneg) i++;
      int startI = i;
      val = 0;
      int scale = 1;
      while (i < endI && xml[i] >= '0' && xml[i] <= '9')
        val = val * 10 + (xml[i++] - 48);
      if (i < endI && xml[i] == '.')
      {
        i++;
    ...
    public SvgReader(string filename)
    {
      ...
      using (StreamReader sr = new StreamReader(filename))
        xml = sr.ReadToEnd();
      int i = xml.IndexOf("path d=\"M", StringComparison.Ordinal);
      if (i <= 0) return;
      i += 9;
      int endI = xml.Length;
  ...
  public class SvgWriter
  {
    ...
    private static readonly RectD rectMax = Clipper.InvalidRectD;
    public static RectD RectMax => rectMax;

    private static readonly RectD rectEmpty = new RectD(true);
