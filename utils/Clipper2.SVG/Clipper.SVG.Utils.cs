...
*                                                                              *
* C# port of CPP/Utils/clipper.svg.utils.h                                     *
*******************************************************************************/
...
#endif
{
  public static class SvgUtils
  {
    ...
    public static void AddSolution(SvgWriter svg, Paths64 paths, bool showCoords)
    {
      svg.AddClosedPaths(paths, 0x4080ff9C, 0x80999999, 1.5, showCoords);
    }

    public static void AddRCSolution(SvgWriter svg, Paths64 paths, bool showCoords)
    {
      foreach (Path64 path in paths)
        svg.AddClosedPath(path, RandomColor(), 0x80999999, 1.0, showCoords);
    }

    public static void AddOpenSolution(SvgWriter svg, Paths64 paths, bool showCoords)
    {
      svg.AddOpenPaths(paths, 0xFF003300, 2.2, showCoords);
    }

    public static void AddSolution(SvgWriter svg, PathsD paths, bool showCoords)
    {
      svg.AddClosedPaths(paths, 0x4080ff9C, 0x80999999, 1.5, showCoords);
    }

    public static void AddRCSolution(SvgWriter svg, PathsD paths, bool showCoords)
    {
      foreach (PathD path in paths)
        svg.AddClosedPath(path, RandomColor(), 0x80999999, 1.0, showCoords);
    }

    public static void AddOpenSolution(SvgWriter svg, PathsD paths, bool showCoords)
    {
      svg.AddOpenPaths(paths, 0xFF003300, 2.2, showCoords);
    }

    public static void SaveToFile(SvgWriter svg,
      string filename, FillRule fillRule,
      int maxWidth = 0, int maxHeight = 0, int margin = 0)
    {
      if (File.Exists(filename)) File.Delete(filename);
      svg.FillRule = fillRule;
      svg.SaveToFile(filename, maxWidth, maxHeight, margin);
    }
  }
}
