...
*                                                                              *
* C# port of CPP/Utils/ClipFileLoad.* and CPP/Utils/ClipFileSave.*             *
*******************************************************************************/
...
#endif
{
  ...
  public static class ClipperFileIO
  {
    ...
    public static void SaveClippingOp(string filename, Paths64? subj,
      Paths64? subjOpen, Paths64? clip, ClipType ct, FillRule fillRule, bool append)
    {
      StreamWriter writer;
      try
      {
      ...
      using (writer)
      {
        ...
        if (subjOpen != null && subjOpen.Count > 0)
        {
          writer.Write("SUBJECTS_OPEN\r\n");
          foreach (Path64 p in subjOpen)
          {
            foreach (Point64 ip in p)
              writer.Write("{0},{1} ", ip.X, ip.Y);
            writer.Write("\r\n");
